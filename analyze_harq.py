            ...
            if 'Discarding DL HARQ' in line:
                ...
                if rnti_match and h_id_match:
                    discard_data.append({
                        ...
                        'rnti': rnti_match.group(1),
                        'h_id': int(h_id_match.group(1)),
                        'slot': float(slot_match.group(1)) if slot_match else None,
                        'tbs': int(tbs_match.group(1)) if tbs_match else None,
...
def identify_interference_slots(harq_df, window_size=10, nack_threshold=0.3):
    ...
    harq_df = harq_df.sort_values('slot').reset_index(drop=True)
    interference_slots = []
    ...
    for i in range(len(harq_df) - window_size + 1):
        ...
        if nack_ratio >= nack_threshold:
            interference_slots.extend(window['slot'].tolist())
...
def plot_harq_timeline(harq_df, output_file='harq_timeline.png'):
    ...
    harq_df = harq_df.sort_values('slot')
    ...
    for ack_val in [0, 1, 2]:
        ...
        ax1.scatter(subset['slot'], [ack_val]*len(subset), 
    ...
    ax2.plot(harq_df['slot'].values, harq_df['nack_rate_ma'].values, 'b-', linewidth=2)
...
def plot_harq_comprehensive(harq_df, pdsch_df, discard_df, stats, output_dir='.'):
    ...
    if not harq_df.empty:
        harq_sorted = harq_df.sort_values('slot')
        ...
        for ack_val in [0, 1, 2]:
            ...
            if not subset.empty:
                ax3.scatter(subset['slot'], [ack_val]*len(subset), 
    ...
    if not harq_df.empty:
        window = 100
        harq_sorted = harq_df.sort_values('slot').copy()
        ...
        ax4.plot(harq_sorted['slot'].values, harq_sorted['nack_rate_ma'].values, 'b-', linewidth=2, label='NACK Rate')
...
def export_interference_report(harq_df, interference_slots, output_file='interference_report.csv'):
    ...
    interference_data = harq_df[harq_df['slot'].isin(interference_slots)].copy()
    interference_data = interference_data.sort_values('slot')
...
def main():
    ...
    # Analyze HARQ performance with retransmission tracking
    ...
    if not harq_df.empty:
        ...
        harq_df.to_csv(harq_csv, index=False)
