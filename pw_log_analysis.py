#!/usr/bin/env python3
"""
Pulse Width Log Analysis Script
Analyzes throughput vs radar pulse width (PW) from OTA experiment logs
"""
...
def analyze_logs_folder(base_folder):
    ...
    for log_folder in log_folders:
        ...
        # Read radar parameters from existing CSV
        radar_csvs = [f for f in os.listdir(log_path) if 'radar_params' in f and f.endswith('.csv')]
        
        if radar_csvs:
            radar_csv_path = os.path.join(log_path, radar_csvs[0])
            radar_df = pd.read_csv(radar_csv_path)
            if len(radar_df) > 0:
                pw = radar_df['PW'].iloc[0] if 'PW' in radar_df.columns else None
                ...
                pw, gain, prf = None, None, None
        else:
            pw, gain, prf = None, None, None
        
        # Find iperf CSV
        iperf_csvs = [f for f in os.listdir(log_path) if 'iperf' in f.lower() and f.endswith('.csv') and 'iperf3' not in f.lower()]
        
        if iperf_csvs:
            iperf_csv_path = os.path.join(log_path, iperf_csvs[0])
            avg_throughput = read_iperf_csv(iperf_csv_path)
            
            if pw is not None and avg_throughput is not None:
                results.append({
                    'folder': log_folder,
                    'pw': pw * 1e6,  # Convert to microseconds for easier reading
                    'gain': gain,
                    'prf': prf,
                    'avg_throughput_mbps': avg_throughput
                })
                print(f"  PW: {pw*1e6:.2f} µs, Gain: {gain}, PRF: {prf}, Avg Throughput: {avg_throughput:.2f} Mbps")
        else:
        ...
        if metrics_csvs and pw is not None:
            ...
            for brate in dl_brates:
                results.append({
                    'folder': log_folder,
                    'pw': pw * 1e6,  # Convert to microseconds
                    'gain': gain,
                    'prf': prf,
...
def plot_throughput_vs_pw(results_df, output_dir):
    """Plot throughput vs radar pulse width"""
    if results_df.empty:
    ...
    # Sort by PW
    iperf_df = iperf_df.sort_values('pw')
    
    # Group by PW and calculate statistics
    grouped = iperf_df.groupby('pw')['avg_throughput_mbps'].agg(['mean', 'std', 'count']).reset_index()
    
    # Create figure
    fig, ax = plt.subplots(figsize=(12, 8))
    
    # Plot all individual points
    ax.scatter(iperf_df['pw'], iperf_df['avg_throughput_mbps'], 
              alpha=0.5, s=50, label='Individual runs', color='lightgreen')
    
    # Plot mean with error bars
    ax.errorbar(grouped['pw'], grouped['mean'], yerr=grouped['std'],
               fmt='o-', linewidth=2, markersize=8, capsize=5,
               label='Mean ± Std Dev', color='darkgreen')
    
    ax.set_xlabel('Radar Pulse Width (µs)', fontsize=14)
    ax.set_ylabel('Average Throughput (Mbps)', fontsize=14)
    ax.set_title('5G Throughput vs Radar Pulse Width', fontsize=16, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=12)
    
    # Add text with total number of runs
    total_runs = len(iperf_df)
    ax.text(0.02, 0.98, f'Total runs: {total_runs}\nPW values: {len(grouped)}',
    ...
    # Save plot
    plot_path = os.path.join(output_dir, 'throughput_vs_pw.png')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"\nPlot saved to: {plot_path}")
    
    # Save summary statistics
    summary_path = os.path.join(output_dir, 'throughput_vs_pw_summary.csv')
    grouped.to_csv(summary_path, index=False)
    print(f"Summary statistics saved to: {summary_path}")
    
    plt.show()

def plot_dl_brate_vs_pw(results_df, output_dir):
    """Plot downlink bitrate from metrics vs radar pulse width"""
    ...
    # Sort by PW
    dl_df = dl_df.sort_values('pw')
    
    # Group by PW and calculate statistics
    grouped = dl_df.groupby('pw')['dl_brate_mbps'].agg(['mean', 'std', 'count', 'median']).reset_index()
    
    # Create figure
    fig, ax = plt.subplots(figsize=(12, 8))
    
    # Plot all individual points (with transparency due to large number)
    ax.scatter(dl_df['pw'], dl_df['dl_brate_mbps'], 
    ...
    # # Plot median as well
    # ax.plot(grouped['pw'].values, grouped['median'].values, 
    #        'g^--', linewidth=1.5, markersize=6, 
    #        label='Median', zorder=9)
    
    ax.set_xlabel('Radar Pulse Width (µs)', fontsize=14)
    ax.set_ylabel('Downlink Bitrate (Mbps)', fontsize=14)
    ax.set_title('5G Downlink Bitrate vs Radar Pulse Width (from Metrics)', fontsize=16, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=12)
    ...
    # Save plot
    plot_path = os.path.join(output_dir, 'dl_brate_vs_pw.png')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"\nDL Bitrate plot saved to: {plot_path}")
    
    # Save summary statistics
    summary_path = os.path.join(output_dir, 'dl_brate_vs_pw_summary.csv')
    grouped.to_csv(summary_path, index=False)
...
def main():
    parser = argparse.ArgumentParser(description='Analyze throughput vs radar pulse width from OTA logs')
    parser.add_argument('--folder', type=str, default='./radarPW005t5000-5x',
    ...
    if not results_df.empty:
        ...
        # Save all results
        results_path = os.path.join(output_dir, 'all_results_pw.csv')
        results_df.to_csv(results_path, index=False)
        print(f"\nAll results saved to: {results_path}")
        
        # Generate throughput plot
        plot_throughput_vs_pw(results_df, output_dir)
        
        # Generate DL bitrate plot
        plot_dl_brate_vs_pw(results_df, output_dir)
    else:
